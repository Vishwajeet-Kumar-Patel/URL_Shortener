  ...
  return (
    <section className="space-y-6">
      <header className="space-y-2">
        <p className="text-xs uppercase tracking-widest text-blue-600">Communication</p>
        <h1 className="text-3xl font-semibold text-white">Announcements</h1>
        <p className="max-w-3xl text-sm text-slate-500">
          Send production announcements with active-user targeting and built-in retry queue behavior for failed deliveries.
        </p>
      </header>
      ...
      <form className={`${formCardClass} space-y-4`} onSubmit={onSubmit}>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <label className="flex items-center gap-2 text-sm text-slate-600 md:pt-8">
        ...
        <button className={formButtonPrimaryClass} disabled={busy} type="submit">
      ...
      <div className="space-y-3">
        {items.map((a) => (
          <article className="rounded-xl border border-slate-200 bg-white p-4" key={a.id}>
