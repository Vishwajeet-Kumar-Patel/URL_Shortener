    ...
    <section className="space-y-5">
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
        <table className="min-w-full text-sm">
          <thead className="bg-slate-50"><tr><th className="px-3 py-2 text-left">Type</th><th className="px-3 py-2 text-left">Status</th><th className="px-3 py-2 text-left">Amount</th><th className="px-3 py-2 text-left">Date</th></tr></thead>
          <tbody>
            {items.map((i) => (
              <tr className="border-t border-slate-200" key={i.id}><td className="px-3 py-2">{i.type}</td><td className="px-3 py-2">{i.status}</td><td className="px-3 py-2">{i.currency} {i.amount}</td><td className="px-3 py-2 text-slate-500">{new Date(i.createdAt).toLocaleString()}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
